...
func _temporarily_disable(node: Node) -> void:
    if NodeUtil.can_be_disabled(node) and not self._previous_state.has(node):
        self._previous_state[node] = NodeUtil.get_disabled(node)
        NodeUtil.set_disabled(node, true)
    for child in node.get_children():
        self._temporarily_disable(child)

func _back_to_previous_state() -> void:
    for node in self._previous_state:
        NodeUtil.set_disabled(node, self._previous_state[node])
